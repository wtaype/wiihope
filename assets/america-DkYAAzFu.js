/* empty css               */import{w as u,$ as n,b as x,a as $,i as v,e as A}from"./main-BJPK_44-.js";import"./vendor-B2AETYxa.js";let e=[],s=new Map,l=1;const f=9;let h=[...u.america];const I=t=>{const a=v("America/Lima"),i=v(t.zona);if(!a?.gmt||!i?.gmt)return"";const c=r=>parseInt(r.match(/GMT([+-]?\d+)/)?.[1]||0),o=c(i.gmt)-c(a.gmt);return o===0?'<i class="fas fa-exchange-alt"></i> Misma hora':o>0?`<i class="fa-solid fa-rotate-right"></i> +${o}h`:`<i class="fa-solid fa-rotate-left"></i> ${o}h`},E=()=>{let t="";for(let a=0;a<60;a++){const i=a*6,c=a%5===0?"wihora_mark wihora_mark_hour":"wihora_mark wihora_mark_minute";t+=`<div class="${c}" style="transform: rotate(${i}deg) translateY(-60px)"></div>`}return t},M=(t,a,i,c)=>{const o=t.data("zona");s.has(o)&&cancelAnimationFrame(s.get(o));const d=requestAnimationFrame(()=>{const w=c*6,p=i*6+c*.1,b=a%12*30+i*.5;t.find(".wihora_hand_second").css("transform",`translateX(-50%) rotate(${w}deg)`),t.find(".wihora_hand_minute").css("transform",`translateX(-50%) rotate(${p}deg)`),t.find(".wihora_hand_hour").css("transform",`translateX(-50%) rotate(${b}deg)`)});s.set(o,d)},g=(t,a)=>{try{const i=v(t.zona);if(!i)return;const[c,o,r]=i.hora.split(":").map(Number),d=A(i.hora),w=I(t);a.removeClass("es-dia es-noche").addClass(d?"es-dia":"es-noche"),M(a,c,o,r),a.find(".wihora_time").text(i.hora),a.find(".wihora_fecha").text(i.fecha),a.find(".wihora_gmt").text(i.gmt),a.find(".wihora_diferencia").html(w),a.find(".wihora_estacion").text(i.estacion),a.find(".wihora_icon i").attr("class",`fas ${d?"fa-sun":"fa-moon"}`)}catch(i){console.error("❌ Error actualizando reloj:",i)}},m=()=>{const t=(l-1)*f,a=t+f,i=h.slice(t,a),c=E();return i.length===0?`<div class="wicont_empty">
  ...
  `).join("")},_=()=>{const t=Math.ceil(h.length/f);if(t<=1)return"";let a="";const i=5;let c=Math.max(1,l-Math.floor(i/2)),o=Math.min(t,c+i-1);o-c<i-1&&(c=Math.max(1,o-i+1));for(let r=c;r<=o;r++)a+=`<button class="wipag_btn ${r===l?"active":""}" data-pagina="${r}">${r}</button>`;return`
  ...
  `},j=async()=>`
    <div class="wicontinente">
      <div class="wicont_header">
        <h1 class="wicont_title">🌏 America</h1>
        ...
        <div class="wicont_controls">
          <div class="wicont_search">
            ...
            <input type="text" id="searchAmerica" placeholder="Buscar ciudad o país..." class="wisearch_input" />
            <span class="wicont_count">${h.length} ciudades</span>
          </div>
        </div>
      </div>
      
      <div class="wicont_grid" id="americaGrid">
        ${m()}
      </div>
  ...
  `,C=()=>{e.forEach(clearInterval),e=[],s.forEach(a=>cancelAnimationFrame(a)),s.clear();const t=()=>{n(".wicont_card_compact").each((a,i)=>{const c=n(i),o=c.data("zona"),r=h.find(d=>d.zona===o);if(r){g(r,c);const d=setInterval(()=>g(r,c),1e3);e.push(d)}})};t(),n("#searchAmerica").on("input",function(){const a=n(this).val().trim();h=a?x(a,"america"):[...u.america],l=1,e.forEach(clearInterval),e=[],s.forEach(i=>cancelAnimationFrame(i)),s.clear(),n("#americaGrid").html(m()),n(".wicont_count").text(`${h.length} ciudades`),n(".wicont_footer").html(_()),t()}),n(document).on("click",".wipag_btn:not(.wipag_prev):not(.wipag_next)",function(){n(this).hasClass("active")||(l=parseInt(n(this).data("pagina")),e.forEach(clearInterval),e=[],s.forEach(a=>cancelAnimationFrame(a)),s.clear(),n("#americaGrid").html(m()),n(".wicont_footer").html(_()),t(),n(".wicont_grid").get(0).scrollIntoView({behavior:"smooth",block:"start"}))}),n(document).on("click",".wipag_prev",()=>{l>1&&(l--,e.forEach(clearInterval),e=[],s.forEach(a=>cancelAnimationFrame(a)),s.clear(),n("#americaGrid").html(m()),n(".wicont_footer").html(_()),t(),n(".wicont_grid").get(0).scrollIntoView({behavior:"smooth",block:"start"}))}),n(document).on("click",".wipag_next",()=>{const a=Math.ceil(h.length/f);l<a&&(l++,e.forEach(clearInterval),e=[],s.forEach(i=>cancelAnimationFrame(i)),s.clear(),n("#americaGrid").html(m()),n(".wicont_footer").html(_()),t(),n(".wicont_grid").get(0).scrollIntoView({behavior:"smooth",block:"start"}))}),console.log("🌏 America inicializado con relojes analógicos")},G=()=>{e.forEach(clearInterval),e=[],s.forEach(t=>cancelAnimationFrame(t)),s.clear()};export{G as cleanup,C as init,j as render};

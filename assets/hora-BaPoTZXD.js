import{S as _,w as m,a as u,$ as d,i as h,e as g}from"./main-Dbqe6aoX.js";import"./vendor-B2AETYxa.js";import"./firebase-Df3JCjk-.js";let n=[],e=new Map;const p=s=>{if(s.ciudad==="Lima")return'<i class="fas fa-map-marker-alt"></i> Tu Ciudad';const i=h("America/Lima"),a=h(s.zona);if(!i?.gmt||!a?.gmt)return"";const r=t=>parseInt(t.match(/GMT([+-]?\d+)/)?.[1]||0),o=r(a.gmt)-r(i.gmt);return o===0?'<i class="fas fa-exchange-alt"></i> Misma hora':o>0?`<i class="fa-solid fa-rotate-right"></i> +${o}h`:`<i class="fa-solid fa-rotate-left"></i> ${o}h`},z=()=>{let s="";for(let i=0;i<60;i++){const a=i*6,r=i%5===0?"wihora_mark wihora_mark_hour":"wihora_mark wihora_mark_minute";s+=`<div class="${r}" style="transform: rotate(${a}deg) translateY(-90px)"></div>`}return s},$=(s,i,a,r)=>{const o=s.data("zona");e.has(o)&&cancelAnimationFrame(e.get(o));const c=requestAnimationFrame(()=>{const l=r*6,v=a*6+r*.1,w=i%12*30+a*.5;s.find(".wihora_hand_second").css("transform",`translateX(-50%) rotate(${l}deg)`),s.find(".wihora_hand_minute").css("transform",`translateX(-50%) rotate(${v}deg)`),s.find(".wihora_hand_hour").css("transform",`translateX(-50%) rotate(${w}deg)`)});e.set(o,c)},f=(s,i)=>{try{const a=h(s.zona);if(!a)return;s.ciudad==="Lima"&&d(".wihora_title").text(`${_()} Gorila`);const[r,o,t]=a.hora.split(":").map(Number),c=g(a.hora),l=p(s);i.removeClass("es-dia es-noche").addClass(c?"es-dia":"es-noche"),$(i,r,o,t),i.find(".wihora_time").text(a.hora),i.find(".wihora_fecha").text(a.fecha),i.find(".wihora_gmt").text(a.gmt),i.find(".wihora_estacion").text(a.estacion),i.find(".wihora_diferencia").html(l),i.find(".wihora_icon i").attr("class",`fas ${c?"fa-sun":"fa-moon"}`)}catch(a){console.error(`❌ Error en ${s.ciudad}:`,a.message)}},j=async()=>{const s=m.principales,i=z();return`
  ...
  `},I=()=>{n.forEach(clearInterval),n=[],d(".wihora_card").each((s,i)=>{const a=d(i),r=a.data("zona"),o=m.principales.find(t=>t.zona===r);if(o){f(o,a);const t=setInterval(()=>f(o,a),1e3);n.push(t)}}),console.log("🕐 Hora inicializado (24h)")},C=()=>{n.forEach(clearInterval),n=[],e.forEach(s=>cancelAnimationFrame(s)),e.clear()};export{C as cleanup,I as init,j as render};

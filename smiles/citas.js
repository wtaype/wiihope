...
import { getls, savels, removels, Notificacion, abrirModal, wiDate, cerrarTodos, Mensaje, wiSpin } from './widev.js';

let smile = getls('wiSmile');
let frases = [];
let fechadb = wiDate(Timestamp);

// 🎨 RENDERIZAR (Declarar primero)
const renderFrases = () => {
...
// 🚀 AUTO-EJECUCIÓN (Ahora después de renderFrases)
(() => {
  const cache = getls('wiFrases');
  if (cache) { frases = cache; renderFrases(); }
  
  onSnapshot(query(collection(db, 'wicitas'), orderBy('creado', 'desc')), (snap) => {
...
// 💾 GUARDAR
$(document).on('click', '.md_guardar', async function() {
...
// ⭐ FAVORITO
$(document).on('click', '.btn_favorito', async function(e) {
  e.stopPropagation();
  const id = $(this).data('id');
...
$(document).on('click', '.btn_eliminar', async function(e) {
  ...
  try {
    ...
    $(this).closest('.phr').fadeOut(300, () => $(this).remove());
    removels('wiFrases'); Notificacion('Frase eliminada', 'success');
